    ...
    <div className="w-full overflow-x-auto custom-scrollbar pb-2">
      <div className="min-w-[500px] relative mt-6 px-1">
        <div className="flex border-b border-gray-200 pb-4 mb-4">
          <div className="w-1/4 font-bold text-[10px] text-gray-500 uppercase tracking-wider pl-2">Task / Phase</div>
          <div className="w-3/4 flex relative">
            {Array.from({ length: totalMonths }).map((_, i) => (
              <div key={i} className="flex-1 text-[10px] text-center font-bold text-gray-400 border-l border-dashed border-gray-200 h-full">M{i+1}</div>
            ))}
          </div>
        </div>
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6 animate-fade-in-up">
        ...
        <div className="glass-panel p-8 rounded-3xl border border-white shadow-xl shadow-indigo-900/5 relative overflow-hidden group bg-gradient-to-br from-white to-indigo-50">
          ...
          <div className="flex justify-between items-start relative z-10">
             <div>
                <p className="text-[11px] font-bold uppercase tracking-widest text-slate-500 mb-2">Feasibility Index</p>
                <div className="flex items-baseline gap-3">
                  <h2 className={`text-7xl font-black ${scoreColor} tracking-tighter drop-shadow-sm`}>{result.overallScore}</h2>
                  <span className="text-lg font-bold text-slate-400">/100</span>
                </div>
             </div>
             <div className="p-4 bg-white shadow-md rounded-2xl text-indigo-600 border border-indigo-50">
               <Icons.Chart />
             </div>
          </div>
          ...
          <p className="text-sm mt-4 text-slate-700 font-semibold leading-relaxed">{result.overallScore > 65 ? 'Project shows viability. Focus on mitigating listed risks.' : 'Significant restructuring required for grant success.'}</p>
        </div>

        {/* Sentiment Card */}
        <div className="glass-panel p-8 rounded-3xl border border-white shadow-xl shadow-blue-900/5 hover:-translate-y-1 transition-all duration-300 bg-gradient-to-br from-white to-blue-50 flex flex-col justify-between">
           <div className="flex justify-between items-start">
              ...
              <div className="p-3 bg-blue-100 text-blue-700 rounded-2xl"><Icons.Users /></div>
           </div>
        ...
        {/* Sustainability Card */}
        <div className="glass-panel p-8 rounded-3xl border border-white shadow-xl shadow-emerald-900/5 hover:-translate-y-1 transition-all duration-300 bg-gradient-to-br from-white to-emerald-50 flex flex-col justify-between">
           <div className="flex justify-between items-start">
              <div>
              ...
              <div className="p-3 bg-emerald-100 text-emerald-700 rounded-2xl"><Icons.Trending /></div>
           </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 animate-fade-in-up delay-100">
        <div className="xl:col-span-2 glass-panel p-8 rounded-3xl shadow-lg border border-white bg-white">
          ...
          <div className="prose prose-sm prose-slate max-w-none text-slate-700 font-medium leading-7 bg-slate-50 p-8 rounded-2xl border border-slate-200 shadow-inner">
            {result.narrative}
          </div>
        </div>

        <div className="glass-panel p-8 rounded-3xl shadow-lg border border-white bg-gradient-to-b from-white to-emerald-50/50 flex flex-col">
          <h3 className="text-lg font-bold text-slate-800 mb-6 flex items-center gap-3">
            <span className="p-2 bg-emerald-100 rounded-xl text-emerald-700 shadow-sm"><Icons.Shield /></span>
            Key Wins
          </h3>
      ...
      {/* 3. Gantt Chart */}
      <div className="glass-panel p-8 rounded-3xl shadow-lg border border-white animate-fade-in-up delay-200 bg-white">
         <div className="flex items-center justify-between mb-2">
            <h3 className="text-lg font-bold text-slate-800 flex items-center gap-3">
                <span className="p-2 bg-blue-100 rounded-xl text-blue-700 shadow-sm"><Icons.Calendar /></span>
                Implementation Schedule
            </h3>
            <span className="px-3 py-1 bg-slate-100 text-slate-600 rounded-full text-[10px] font-bold uppercase tracking-wider">Timeline Forecast</span>
         </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-fade-in-up delay-300">
        <div className="glass-panel p-8 rounded-3xl shadow-lg border border-white h-[420px] flex flex-col bg-white">
          ...
          <div className="flex-grow">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={result.timeline}>
                ...
                <Area type="monotone" dataKey="sentimentScore" stroke="#3b82f6" strokeWidth={4} fill="url(#colorSentiment)" animationDuration={1500} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
           ...
           <div className="flex-grow">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart layout="vertical" data={result.stakeholderAnalysis} margin={{ top: 0, right: 30, left: 30, bottom: 0 }}>
                ...
                <Bar dataKey="sentiment" barSize={24} radius={[6, 6, 6, 6]} animationDuration={1500}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6 animate-fade-in-up delay-400">
           ...
           <div className="flex-grow -ml-4">
            <ResponsiveContainer width="100%" height="100%">
              <RadarChart cx="50%" cy="50%" outerRadius="70%" data={result.metrics}>
                ...
                <Radar name="Score" dataKey="score" stroke="#8b5cf6" strokeWidth={3} fill="#8b5cf6" fillOpacity={0.3} animationDuration={1500} />
        ...
        <div className="xl:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="flex-grow">
                 <ResponsiveContainer width="100%" height="100%">
                    <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 0 }}>
                      ...
                      <Scatter name="Risks" data={result.riskAnalysis} fill="#ef4444" animationDuration={1500}>
            ...
            {/* Critical Risks List */}
            <div className="glass-panel p-8 rounded-3xl shadow-lg border border-white h-[450px] flex flex-col bg-gradient-to-br from-rose-50 to-white">
               ...
               <div className="flex-grow overflow-y-auto custom-scrollbar pr-2 space-y-3">
                 {result.risks?.map((risk, idx) => (
                   <div key={idx} className="p-4 bg-white border border-rose-200 rounded-2xl shadow-sm hover:shadow-md transition-shadow group">
                 ...
                 {(!result.risks || result.risks.length === 0) && (
                    <div className="h-full flex flex-col items-center justify-center text-slate-400">
                        <Icons.Shield />
                        <p className="text-sm mt-2 font-medium">No critical risks identified.</p>
                    </div>
                 )}
               </div>
            </div>
      ...
      {/* 6. Pivots */}
      <div className="glass-panel p-10 rounded-[2.5rem] border border-white animate-fade-in-up delay-500 bg-gradient-to-br from-indigo-50 via-white to-purple-50 shadow-2xl relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {result.pivots?.map((pivot, idx) => (
            <button key={idx} onClick={() => onApplyPivot(pivot)} className="relative text-left bg-white p-8 rounded-3xl shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 group border border-slate-200 flex flex-col h-full overflow-hidden">
              ...
              {pivot.changes && Object.keys(pivot.changes).length > 0 && (
                <div className="mb-6 flex flex-wrap gap-2">
                  {Object.keys(pivot.changes).filter(key => key !== 'description').slice(0, 3).map(key => (
                    <span key={key} className="px-2.5 py-1 rounded-lg bg-slate-50 text-[10px] font-bold text-slate-500 uppercase tracking-wider border border-slate-200 group-hover:border-indigo-100 group-hover:text-indigo-500 group-hover:bg-indigo-50/50 transition-colors">
                        {key.replace(/([A-Z])/g, ' $1').trim()}
                    </span>
                  ))}
                  {Object.keys(pivot.changes).length > 3 && <span className="px-2 py-1 rounded-lg bg-slate-50 text-[10px] text-slate-400 font-bold border border-slate-200">+{Object.keys(pivot.changes).length - 3}</span>}
                </div>
              )}
              
              <div className="w-full pt-4 border-t border-slate-100 mt-auto flex justify-between items-center group-hover:border-indigo-100 transition-colors">
